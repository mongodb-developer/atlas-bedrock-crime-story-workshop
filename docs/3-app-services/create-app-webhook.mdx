    ...
    if (currentStage.targetObjectIds.includes(evidence[0]._id.toString())) {
      ...
      // Return the sulotion after 5 attempts
       if(progress.retry > 4){
         ...
         prompt = ` [INST] Don't output more than one simple sentence User did not get the needed document rejected by ${currentStage.rejection}, user retries is at ${progress.retry} out of 5, [/INST].
