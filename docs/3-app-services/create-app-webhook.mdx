  ...
  try {
    ...
    if (currentStage.targetObjectIds.includes(evidence[0]._id.toString())) {
       console.log("Document Found")
        ...
        prompt = ` Act as a crime investigator, Take the input document: ${JSON.stringify(evidence[0])} and summarize the findings`;
      success = true;
     
    } else {
      // Return the sulotion after 5 attempts
       if(progress.retry > 4){
          return [{ "document" : currentStage.solution, "imgUrl" : 'https://soundscout.s3.eu-central-1.amazonaws.com/lightball.png' }]
        }
         prompt = ` use one sentence and Act as a crime investigator, User did not get the needed document. You must first complete: ${currentStage.expectedDirection}  since you ended at: ${currentStage.rejection}. `;
    }
    ...
    if (success) {
      const retVal = {"document": nextMove.generations[0].text};
    ...
    return [{"text": nextMove.generations[0].text}];
...
Finally, make sure that the function created (eg. getSearch) is on "SYSTEM" privilege for this demo.
   <img
        alt="Security quickstart page highlighting the 'Database' tab in the left-hand menu"
...
1. Click "Insert Data" or "Add My Own Data" and switch to the document view. Paste the content of the ["evidence.json"](https://github.com/mongodb-developer/atlas-bedrock-crime-story-demo) file from the "data" folder in the repository. 
1. Click "Insert" and wait for the data to be imported.
 <img
        ...
        border="1"
    />
1. Repeat the same process for the ["story.json"](https://github.com/mongodb-developer/atlas-bedrock-crime-story-demo) file in the "data" folder.
