...
exports = async function({body}, response) {
  ...
  try {
    ...
    console.log(evidence[0]._id);
    console.log(JSON.stringify(currentStage.targetObjectIds));
    if (currentStage.targetObjectIds.includes(evidence[0]._id.toString())) {
       console.log("Document Found")
     
        prompt = `Use one sentence Act as crime like Sherlock Holmes, Take the input document: ${JSON.stringify(evidence[0])} and summarize the findings that helps ${currentStage.expectedDirection}`;
      success = true;
      ...
      // Return the sulotion after 5 attempts
       if(progress.retry > 4){
         ...
         prompt = ` Use one sentence and Act as crime like Sherlock Holmes. User did not get the needed document, user retries is at ${progress.retry} out of 5, give a hint to reach ${currentStage.solution}
         using expected direction of :  ${currentStage.expectedDirection} but start with slight hint on 1 and strong on 4.`// You must first complete: ${currentStage.expectedDirection}  since you ended at: ${currentStage.rejection}. `;
    }
    ...
    console.log(`Generation: ${nextMove.generations[0].text}`);
    if (success) {
      ...
      await await playerProgress.updateOne({player: player}, {"$inc": {"stage": 1}});
...
1. Click "Insert Data" or "Add My Own Data" and switch to the document view. Paste the content of the ["evidence.json"](https://raw.githubusercontent.com/mongodb-developer/atlas-bedrock-crime-story-demo/main/data/evidence.json) file from the "data" folder in the repository. 
1. Click "Insert" and wait for the data to be imported.
 <img
        ...
        border="1"
    />
1. Repeat the same process for the ["story.json"](https://raw.githubusercontent.com/mongodb-developer/atlas-bedrock-crime-story-demo/main/data/story.json) file in the "data" folder to load into "Story".
